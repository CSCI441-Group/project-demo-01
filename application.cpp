...
/*int orderwindow(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR args, int ncmdshow)
{
	WNDCLASSW kOrders = { 0 };
	HWND hWnd;
...
int addorderscreen(HWND hWnd, WPARAM wp)
{
	CreateWindowW(L"STATIC", L"PUT ORDER INFO HERE", WS_VISIBLE | WS_CHILD, 0, 0, 300, 400, hWnd, NULL, NULL, NULL);
}
...
void Application::updateOrderAsPlaced()
{
    ...
    if (rc == ReturnCode::Success)
    {
		...
		//Take order info and output to window as child windows.
		//MainWindow(hWnd, msg, wp, lp);
        pressToContinue();
        return;
    }
    else if (rc == ReturnCode::NonexistentId)
